...
function fsel () {
    ...
    if [ $? = 0 ]; then
        INFO="Le fichier sélectionné est `cat $FICHTMP`"
    else 
        INFO="Vous n'avez pas sélectionné de fichier!"
    fi
...
function text () {
    ...
    Utilisez les 4 flèches pour vous déplacer.
    " 14 60
...
function inpu () {
    ...
    if [ $? = 0 ]; then
        INFO="Votre nom de login est `cat $FICHTMP`"
    else 
        INFO="Vous n'avez pas de nom de login!"
    fi
...
function pass () {
    ...
    if [ $? = 0 ]; then
        INFO="Le mot de passe choisi est `cat $FICHTMP`"
    else
        INFO="Vous n'avez pas de mot de passe!"
    fi
...
function time () {
    ...
    if [ $? = 0 ]; then
        INFO="Votre heure locale est `cat $FICHTMP`"
    else
        INFO="Vous n'avez pas défini d'heure.
